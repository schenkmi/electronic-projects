classdef AudioAttenuatorAppV3 < handle
    ...
    methods
        function app = AudioAttenuatorAppV3()
            app.Data = struct();
            app.PositionData = table();
            app.buildUI();
            app.calculate();
        end
    ...
    methods (Access = private)
        ...
        function buildUI(app)
            ...
            g = uigridlayout(app.InputPanel,[11 2]);
            g.RowHeight = {30,26,30,30,30,30,30,30,30,35,'1x'};
            ...
            app.MatchCheckBox = uicheckbox(g, ...
                'Text','Match source/load to stage resistance (ideal)', ...
                'Value',true);
            app.MatchCheckBox.Layout.Row = 2; app.MatchCheckBox.Layout.Column = [1 2];

            uilabel(g,'Text','Source resistance (Ω):');
            app.SourceREdit = uieditfield(g,'numeric','Value',10000, ...
                'Limits',[0 Inf],'Enable','off');
            app.SourceREdit.Layout.Row = 3; app.SourceREdit.Layout.Column = 2;

            uilabel(g,'Text','Load resistance (Ω):');
            app.LoadREdit = uieditfield(g,'numeric','Value',10000, ...
                'Limits',[0 Inf],'Enable','off');
            app.LoadREdit.Layout.Row = 4; app.LoadREdit.Layout.Column = 2;

            uilabel(g,'Text','Stage attenuation (dB):');
            app.StepEdit = uieditfield(g,'numeric','Value',1.0, ...
                'Limits',[0.001 100]);
            app.StepEdit.Layout.Row = 5; app.StepEdit.Layout.Column = 2;

            uilabel(g,'Text','Resistor series:');
            app.SeriesDropDown = uidropdown(g, ...
                'Items',{'E6','E12','E24','E48','E96'}, ...
                'Value','E96');
            app.SeriesDropDown.Layout.Row = 6; app.SeriesDropDown.Layout.Column = 2;
            ...
            note = uilabel(g,'Text',sprintf([ ...
                ...
                'Input/output impedance is terminated with the source/load ' ...
                'resistance above, not necessarily the stage resistance. ' ...
                'The per-stage dB values still assume the matched (stage-' ...
                'resistance) condition, since that is what the constant-' ...
                'resistance design targets.']));
            ...
            % Graph panel
            % -------------------------------------------------------------
            app.GraphPanel = uipanel(outer,'Title','Performance and impedance');
            app.GraphPanel.Layout.Column = 3;

            pg = uigridlayout(app.GraphPanel,[4 2]);
            pg.RowHeight = {'1x','1x',190,'1x'};
            pg.ColumnWidth = {'1x','1x'};
            pg.Padding = [10 10 10 10];

            app.AttAxes = uiaxes(pg);
            app.AttAxes.Layout.Row = 1;
            app.AttAxes.Layout.Column = [1 2];
            ...
            app.ErrorAxes.Layout.Row = 2;
            app.ErrorAxes.Layout.Column = [1 2];
            ...
            app.ImpedancePanel = uipanel(pg,'Title','Per-position input/output impedance');
            app.ImpedancePanel.Layout.Row = 3;
            app.ImpedancePanel.Layout.Column = [1 2];

            ig = uigridlayout(app.ImpedancePanel,[1 2]);
            ig.ColumnWidth = {'1x','1x'};
            ig.Padding = [5 5 5 5];

            app.ImpedanceTable = uitable(ig);
            app.ImpedanceTable.RowName = [];
            app.ImpedanceTable.ColumnEditable = false;
            app.ImpedanceTable.ColumnName = {'Position','Ideal dB','Actual dB','Input R (ohm)','Output R (ohm)','Error (dB)'};

            app.ImpedanceInfoLabel = uilabel(ig,'Text','');
            app.ImpedanceInfoLabel.VerticalAlignment = 'top';
            app.ImpedanceInfoLabel.WordWrap = 'on';

            impedancePlots = uipanel(pg,'Title','Input / output impedance');
            impedancePlots.Layout.Row = 4;
            impedancePlots.Layout.Column = [1 2];

            zg = uigridlayout(impedancePlots,[1 2]);
            zg.ColumnWidth = {'1x','1x'};
            zg.Padding = [5 5 5 5];

            app.InputRAxes = uiaxes(zg);
            title(app.InputRAxes,'Input resistance');
            xlabel(app.InputRAxes,'Position');
            ylabel(app.InputRAxes,'Rin (ohm)');
            grid(app.InputRAxes,'on');

            app.OutputRAxes = uiaxes(zg);
            title(app.OutputRAxes,'Output resistance');
            xlabel(app.OutputRAxes,'Position');
            ylabel(app.OutputRAxes,'Rout (ohm)');
            grid(app.OutputRAxes,'on');

            app.REdit.ValueChangedFcn = @(src,event)app.calculate();
            app.SourceREdit.ValueChangedFcn = @(src,event)app.calculate();
            app.LoadREdit.ValueChangedFcn = @(src,event)app.calculate();
            app.MatchCheckBox.ValueChangedFcn = @(src,event)app.calculate();
        ...
        function calculate(app)
            try
                ...
                if app.MatchCheckBox.Value
                    app.SourceREdit.Value = R;
                    app.LoadREdit.Value = R;
                    app.SourceREdit.Enable = 'off';
                    app.LoadREdit.Enable = 'off';
                else
                    app.SourceREdit.Enable = 'on';
                    app.LoadREdit.Enable = 'on';
                end
                sourceR = app.SourceREdit.Value;
                loadR = app.LoadREdit.Value;
                ...
                for p = 1:numel(positions)
                    ...
                    actualPosDB(p) = sum(reshape(bits,1,[]) .* reshape(actualDB,1,[]));
                end
                ...
                % Full per-position impedance from the rounded resistor
                % network.  The permutation determines physical stage order.
                if contains(perm,'MSB')
                    physicalMap = N:-1:1;
                else
                    physicalMap = 1:N;
                ...
                for p = 1:numel(positions)
                    codeBits = reshape(bitget(positions(p),1:N),1,[]);
                    physicalBits = codeBits(physicalMap);
                    physicalRs = Rs(physicalMap);
                    physicalRp = Rp(physicalMap);
                    [Zin(p),Zout(p)] = app.ladderImpedance( ...
                        physicalRs,physicalRp,loadR,sourceR,physicalBits,topology);
                end

                % Display table in the selected permutation.
                % For LSB->MSB, display is already ascending; the table is
                % explicitly shown MSB first to match JOS-style diagrams.
                if contains(perm,'MSB')
                    order = N:-1:1;
                else
                    order = 1:N;
                ...
                stageCol = (1:N)';
                fmtR = @(v) arrayfun(@(x) sprintf('%.6g',x),v,'UniformOutput',false);
                T = table( ...
                    stageCol, ...
                    nominalDB(order)', ...
                    fmtR(idealRs(order)'),fmtR(Rs(order)'), ...
                    fmtR(idealRp(order)'),fmtR(Rp(order)'), ...
                    actualDB(order)', ...
                ...
                info = sprintf([ ...
                    'Attenuator: %s\n' ...
                    'Stage resistance: %s\n' ...
                    'Source resistance: %s   |   Load resistance: %s\n' ...
                    ...
                    topology,app.formatValue(R), ...
                    app.formatValue(sourceR),app.formatValue(loadR), ...
                    N,2^N,max(idealPosDB),A, ...
                    max(abs(positionError)),maxDeviation, ...
                    min(Zin),max(Zin),min(Zout),max(Zout));

                app.InfoLabel.Text = info;

                app.ImpedanceTable.Data = table( ...
                    positions',idealPosDB',actualPosDB',fmtR(Zin'),fmtR(Zout'),positionError', ...
                    'VariableNames',{'Position','Ideal_dB','Actual_dB', ...
                    'Input_R_ohm','Output_R_ohm','Error_dB'});

                app.ImpedanceInfoLabel.Text = sprintf([ ...
                    'Input R:  %.6g to %.6g ohm\n' ...
                    'Output R: %.6g to %.6g ohm\n' ...
                    'Terminated with source %s / load %s.\n' ...
                    'Calculated for all %d binary positions from the rounded ' ...
                    'E-series resistor network.'], ...
                    min(Zin),max(Zin),min(Zout),max(Zout), ...
                    app.formatValue(sourceR),app.formatValue(loadR), ...
                    numel(positions));
                ...
                cla(app.InputRAxes);
                plot(app.InputRAxes,positions,Zin,'o-','LineWidth',1.0);
                grid(app.InputRAxes,'on');

                cla(app.OutputRAxes);
                plot(app.OutputRAxes,positions,Zout,'o-','LineWidth',1.0);
                grid(app.OutputRAxes,'on');

                app.PositionData = table( ...
                    positions',idealPosDB',actualPosDB',Zin',Zout',positionError', ...
                    'VariableNames',{'Position','Ideal_dB','Actual_dB', ...
                    'Input_R_ohm','Output_R_ohm','Error_dB'});

                app.Data = struct( ...
                    'StageResistance',R, ...
                    'SourceResistance',sourceR, ...
        ...
        function [zin,zout] = ladderImpedance(app,Rs,Rp,loadR,sourceR,bits,topology)
            %#ok<INUSD>
            % Nodes 1..N+1 form a chain. A bypassed stage (bit=0) is
            % electrically transparent: no series resistor and no shunt
            % resistor at all (the attenuation math elsewhere treats it
            % as a plain wire contributing 0 dB), so its two endpoints
            % are literally the same electrical node.
            %
            % Approximating that short with a tiny series resistance (as
            % an earlier version of this function did) creates a
            % conductance many orders of magnitude larger than the rest
            % of the network. Once any stage is bypassed - the common
            % case for almost every position except all-bits-set - the
            % numerical regularization needed to keep that huge value
            % from being singular ends up swamping the legitimate, much
            % smaller conductances elsewhere (e.g. a 10 kΩ termination),
            % corrupting the result. Merging the nodes directly avoids
            % ever creating an artificial extreme value in the first
            % place, so the matrix stays well-conditioned.
            N = numel(Rs);
            ...
            for n = 1:N
                if bits(n)
                    ...
                    G0(b,b) = G0(b,b) + 1/max(Rp(n),1e-12);
                end
            end

            % Rin: test the signal input against the output, terminated
            % by the actual load.
            zin = app.twoTerminalLadderResistance( ...
                G0,loadR,group(1),group(N+1));

            % Rout: test the output against the input, terminated by the
            % actual source resistance.
            zout = app.twoTerminalLadderResistance( ...
                G0,sourceR,group(N+1),group(1));

        end

        function z = twoTerminalLadderResistance(app,G0,R,portNode,referenceNode) %#ok<INUSD>
            % Driving-point resistance looking into portNode of the
            % (already-reduced) conductance matrix G0, with referenceNode
            % additionally terminated by R to ground. A single 1 A test
            % current at portNode is enough: the return current finds
            % its own way to ground through the network's own shunt
            % conductances and the added termination, so Zin/Zout is
            % simply V(portNode) relative to the implicit 0 V ground.
            M = size(G0,1);
            G = G0;
            G(referenceNode,referenceNode) = ...
                G(referenceNode,referenceNode) + 1/max(R,1e-12);
        ...
        function exportCSV(app)
            ...
            T2 = table( ...
                ...
                D.InputResistance',D.OutputResistance', ...
                'VariableNames',{ ...
                'Position','Ideal_dB','Actual_dB','PositionError_dB', ...
                'StepActual_dB','StepError_dB','Input_R_ohm','Output_R_ohm'});
