...
% Output Ripple Analysis Parameters (Real Component Test)
CoutTest = 22.0 * 10^-6; % Actual capacitor chosen for the board (e.g., 22 uF nominal)
CoutEsr = 0.005;         % Typical ESR of a 22uF X7R ceramic cap at 1MHz (5 mOhm)
...
fprintf('Recommended Real C (X7R):     %.1f µF\n', max(22.0, CoutRealRecommended * 1000000));

% 8. New: Output Voltage Ripple Verification
% Assuming effective capacitance drops by ~40% due to DC Bias effect (common for 22uF MLCC at 6.3V)
CoutEffective = CoutTest * 0.60; 
...
fprintf('Tested Output Capacitor:      %.1f µF (Effective Cap due to DC Bias: %.1f µF)\n', CoutTest * 1000000, CoutEffective * 1000000);
fprintf('Capacitive Voltage Ripple:    %.2f mV\n', DeltaV_C * 1000);
fprintf('ESR-Induced Voltage Ripple:   %.2f mV\n', DeltaV_ESR * 1000);
